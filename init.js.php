...
define('CACHE_FILE', 'https://mod.calltouch.ru/init.js');
// Сколько времени файл актуален
define('CACHE_FILE_TIME', 60);
// Префикс для файла кеша
define('CACHE_PREFIX', 'cache_js_calltouch_init_');
...
function cache($url, $min_expiration) {
    ...
    $fn = @tempnam(CACHE_DIR, 'remote_js_calltouch_init_');
    if ($fn === false) {
        return $url_or_file;
    }
    ...
    */
    ...
    $output = str_replace(
        ...
        "{src: \"https://mod.calltouch.ru/init.js\"}",
        $output
    );
